...
seed = randrange(100)
y2_train, y2_test = train_test_split(df, test_size = 0.3, random_state = seed, stratify=df['age']) # stratify=df['loan']
y1_train, y1_test = train_test_split(df, test_size = 0.3, random_state = seed, stratify=df['age']) # 
...
y2_gbc = RandomForestClassifier()
y2_mdl = y2_gbc.fit(y2_X_train, y2_y_train)

y1_gbc = RandomForestClassifier()
